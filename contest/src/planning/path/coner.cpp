...
bool isCorner(const vector<egoPath_struc>& egoPath_vec, const egoPose_struc& egoPose, int closest_idx) {
    ...
    double current_path_angle = atan2(egoPath_vec[closest_idx + 3].n - egoPath_vec[closest_idx].n, 
                                      egoPath_vec[closest_idx + 3].e - egoPath_vec[closest_idx].e);
    
    // 2. 좀 더 앞쪽(예: 15m 앞)의 각도 (미리 코너를 예측하기 위함)
    int look_ahead_idx = findWaypoint(egoPath_vec, egoPose, L_d); 
    double future_path_angle = atan2(egoPath_vec[look_ahead_idx].n - egoPath_vec[look_ahead_idx - 3].n, 
                                     egoPath_vec[look_ahead_idx].e - egoPath_vec[look_ahead_idx - 3].e);
