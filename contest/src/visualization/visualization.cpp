...
void publishEgoCircle(const egoPose_struc& pose, double radius) {
    visualization_msgs::Marker marker;
    marker.header.frame_id = "base_link"; // [수정]
    marker.header.stamp = ros::Time::now();
    marker.ns = "ego_collision_circle";
    marker.id = 0;
    marker.type = visualization_msgs::Marker::CYLINDER;
    marker.action = visualization_msgs::Marker::ADD;

    // [수정] base_link 기준이므로 차는 항상 0, 0에 있습니다.
    ...
    marker.scale.z = 0.1;
    marker.color.a = 0.4; marker.color.g = 1.0;
    marker.lifetime = ros::Duration(0.1);
    ego_circle_pub.publish(marker);
}